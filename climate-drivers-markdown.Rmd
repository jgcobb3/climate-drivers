...
### Missing values
Identifies and groups the independent variables of interest. 
```{r}
...
Created a subset that removed observations with NAs. The number of observations is reduced to 4,760 observations, 15 variables.
```{r}
...
### Now, onto the models!
My goal is to compare the fit of each of the 10 models listed below:
```{r}
list_models <- c("y_var ~ PercLossDrought + stdiv",
                 "y_var ~ PercLossDrought + PDSI_MEAN_2016 | stdiv", 
                 "y_var ~ PercLossDrought + PDSI_MEAN_2014 | stdiv",
                 "y_var ~ PercLossDrought + PDSI_MEAN_2012 | stdiv",
                 "y_var ~ PercLossDrought + PDSI_MEAN_2007 | stdiv",
                 "y_var ~ PercLossDrought + PDSI_MEAN_2002 | stdiv",
                 "y_var ~ PercLossDrought + PDSI_MEAN_2016 | stdiv",
                 "y_var ~ PercLossDrought + PDSI_STD_2014 | stdiv",
                 "y_var ~ PercLossDrought + PDSI_STD_2012 | stdiv",
                 "y_var ~ PercLossDrought + PDSI_STD_2007 | stdiv",
                 "y_var ~ PercLossDrought + PDSI_STD_2002 | stdiv")
```
...
### Missing values
Not 100% sure what this block of code does. 
```{r}
...
### Now, onto the models!
Our goal is to compare the fit of each of the 11 models listed below.
```{r}
list_models <- c("y_var ~ PercLossDrought + Agency",
                 "y_var ~ PercLossDrought + Agency + PDSI_MEAN_2016", 
                 "y_var ~ PercLossDrought + Agency + PDSI_MEAN_2014",
...
Then, I set up the models to run in a future function
```{r}
n_model <- length(list_models)
list_mod <- vector("list", length=n_model)
```
...
>>>>>>> ec5c590ed073f0cb0ab42a338e226b497e633bb0
